...
@app.route('/dash')
def dashb():
    cursor_f=mysql.connection.cursor()
    query_f=cursor_f.execute("SELECT * FROM users")
    if query_f:
        all_users=cursor_f.fetchall()
    ...
    if query3 :
     ...
     return render_template('dashbord.html',data=result,data3=result3,users=all_users) 
    ...
    if request.method=="POST":
          ...
          if query:   
            ...
            if result:
                 ...
                 return render_template("dashbord.html",data=result2,data3=result3,users=all_users)
